...
function renderCluster() {
  ...
  clusterData.forEach((row, i) => {
    ...
    tr.innerHTML = `
      <td contenteditable="true" onblur="clusterData[${i}].river=this.innerText.trim()">${row.river}</td>
      <td contenteditable="true" onblur="clusterData[${i}].cluster=this.innerText.trim()">${row.cluster}</td>
      <td contenteditable="true" style="text-align:left;white-space:pre-wrap;min-width:140px;" onblur="clusterData[${i}].lease=this.innerText.trim()">${row.lease}</td>
      <td>
        <select onchange="clusterData[${i}].location=this.value">
          <option ${row.location==='Riverbed'?'selected':''}>Riverbed</option>
          <option ${row.location==='Patta Land'?'selected':''}>Patta Land</option>
        </select>
      </td>
      <td contenteditable="true" onblur="clusterData[${i}].village=this.innerText.trim()">${row.village}</td>
...
function renderCont() {
  ...
  contData.forEach((row, i) => {
    ...
    const tr = document.createElement('tr');
    tr.innerHTML = `
      <td contenteditable="true" onblur="contData[${i}].river=this.innerText.trim()">${row.river}</td>
      <td contenteditable="true" onblur="contData[${i}].ccNo=this.innerText.trim()">${row.ccNo}</td>
      <td contenteditable="true" onblur="contData[${i}].clusterNo=this.innerText.trim()">${row.clusterNo}</td>
      <td contenteditable="true" onblur="contData[${i}].leases=this.innerText.trim()">${row.leases}</td>
      <td>
        <select onchange="contData[${i}].location=this.value">
          <option ${row.location==='Riverbed'?'selected':''}>Riverbed</option>
          <option ${row.location==='Patta Land'?'selected':''}>Patta Land</option>
        </select>
      </td>
      <td contenteditable="true" onblur="contData[${i}].distance=this.innerText.trim()">${row.distance}</td>
