...
function renderDistrictLegends() {
  ...
  // 1. Sidebar rendering (compact list, always styled for dark navy background)
  if (sidebarEl) {
    let html = '';
    districts.forEach(d => {
      const style = getDistrictStyle(d, true); // Force dark theme styles for dark sidebar
      ...
      const itemStyle = isSelected
        ...
        : `border: 2px solid transparent; color: #cbd5e1;`;
      ...
      const hoverEvents = isSelected
        ...
        : `onmouseover="this.style.background='rgba(255,255,255,0.08)', this.style.borderColor='rgba(255,255,255,0.15)'" onmouseout="this.style.background='transparent', this.style.borderColor='transparent'"`;
        
      html += `
        <div onclick="filterDashboardByDistrict('${d}')" ${hoverEvents} style="display: flex; align-items: center; gap: 8px; padding: 8px 12px; border-radius: var(--r-sm); cursor: pointer; transition: all 0.2s ease; font-size: 12.5px; margin-bottom: 4px; ${itemStyle}">
          <span style="width: 10px; height: 10px; border-radius: 50%; background: ${style.border}; border: 1.5px solid #0d1d36; flex-shrink: 0; box-shadow: 0 0 4px ${style.glow};"></span>
          <span style="flex: 1; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; font-weight: 600;">${d}</span>
